...
class GameState:
    ...
    def reset_round_data(self):
        ...
        self.wolf_kill_confirmed = False # 標記狼人是否已殺完
...
async def start(interaction: discord.Interaction):
    ...
    if current_game.game_type == 'werewolf': min_players = 5 # 測試用可改小
...
async def setup_werewolf_game():
    ...
    # 2. 建立特殊身分頻道 (主持人不可見，只有該身分可見)
    
    # Helper function to create private channel
...
async def post_vote_processing(dead_player):
    ...
    # 檢查勝利條件 (如果遊戲結束，這裡會把 phase 設為 GAME_OVER)

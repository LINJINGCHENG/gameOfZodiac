...
class GamePhase:
    ...
    WEREWOLF_WITCH = 11     # 狼人行動完，女巫行動 (預言家仍可查)
...
async def start(interaction: discord.Interaction):
    ...
    try:
        # 1. 建立公共遊戲室 (使用 view_channel 控制權限)
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False), # 關鍵修正：路人不可見
            guild.me: discord.PermissionOverwrite(view_channel=True),
            host: discord.PermissionOverwrite(view_channel=True, send_messages=True)
        }
...
async def setup_werewolf_game():
    ...
    async def create_role_channel(name, allowed_players):
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False), # 關鍵修正：路人不可見
            guild.me: discord.PermissionOverwrite(view_channel=True),
            host: discord.PermissionOverwrite(view_channel=False) # 主持人不可見
        }
        for p in allowed_players:
            overwrites[p] = discord.PermissionOverwrite(view_channel=True, send_messages=True)
...
async def seer_check(interaction: discord.Interaction, target: discord.Member):
    ...
    # 關鍵修正：允許在 NIGHT 和 WITCH 階段查驗
    if current_game.phase not in [GamePhase.WEREWOLF_NIGHT, GamePhase.WEREWOLF_WITCH]:
